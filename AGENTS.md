...
- Preserve the learning path: Rust essentials → blockchain model and mini-chain → Ethereum/Solidity → Solana/Anchor.
...
The runnable Rust exercise is the Cargo workspace crate under `practice/mini-chain`. Keep it synchronized with `src/ch05-00-mini-chain.md`.
...
- `solidity`, `bash`: examples for their own toolchains
...
Completion requires all six checks to pass. If Foundry or Anchor is unavailable, report Solidity and Anchor snippets as documentation-reviewed but not locally executed.

## Editing rules

- Keep diffs small and remove stale files when restructuring the table of contents.
- Do not commit `book/` or `target/`.
- Prefer official Rust, Ethereum, Solidity, Foundry, OpenZeppelin, Solana, and Anchor documentation for technical claims.
